...
bool STDescManager::MatchPairwise(
    pcl::PointCloud<pcl::PointXYZI>::Ptr source_cloud,
    pcl::PointCloud<pcl::PointXYZI>::Ptr target_cloud,
    std::pair<Eigen::Vector3d, Eigen::Matrix3d> &transform,
    std::vector<std::pair<STDesc, STDesc>> &match_pairs, double &score,
    double &downsample_ms, double &feature_ms, double &correspondence_ms,
    double &registration_ms) {
  auto total_begin = std::chrono::high_resolution_clock::now();
  auto t0 = std::chrono::high_resolution_clock::now();
  if (config_setting_.ds_size_ > 0.01) {
    down_sampling_voxel(*source_cloud, config_setting_.ds_size_);
    down_sampling_voxel(*target_cloud, config_setting_.ds_size_);
  }
  auto t1 = std::chrono::high_resolution_clock::now();
  downsample_ms = time_inc(t1, t0);

  ConfigSetting pair_cfg = config_setting_;
  pair_cfg.skip_near_num_ = 0;
  STDescManager pair_manager(pair_cfg);

  t0 = std::chrono::high_resolution_clock::now();
  std::vector<STDesc> src_stds;
  std::vector<STDesc> tgt_stds;
  pair_manager.GenerateSTDescs(source_cloud, src_stds);
  pair_manager.AddSTDescs(src_stds);
  pair_manager.GenerateSTDescs(target_cloud, tgt_stds);
  t1 = std::chrono::high_resolution_clock::now();
  feature_ms = time_inc(t1, t0);

  t0 = std::chrono::high_resolution_clock::now();
  std::vector<STDMatchList> candidate_matcher_vec;
  pair_manager.candidate_selector(tgt_stds, candidate_matcher_vec);
  t1 = std::chrono::high_resolution_clock::now();
  correspondence_ms = time_inc(t1, t0);
  ...
  for (size_t i = 0; i < candidate_matcher_vec.size(); i++) {
    ...
    std::pair<Eigen::Vector3d, Eigen::Matrix3d> relative_pose;
    std::vector<std::pair<STDesc, STDesc>> sucess_match_vec;
    pair_manager.candidate_verify(candidate_matcher_vec[i], verify_score,
                                  relative_pose, sucess_match_vec);
    if (verify_score > best_score) {
      best_score = verify_score;
      best_transform = relative_pose;
      best_sucess_match_vec = sucess_match_vec;
  ...
  t1 = std::chrono::high_resolution_clock::now();
  registration_ms = time_inc(t1, t0);

  transform = best_transform;
  match_pairs = best_sucess_match_vec;
  score = best_score;
...
void STDescManager::extract_corner(
  ...
  std::vector<std::vector<std::vector<Eigen::Vector2d>>> img_container(
      x_axis_len, std::vector<std::vector<Eigen::Vector2d>>(y_axis_len));
  std::vector<std::vector<double>> img_count_array(
      x_axis_len, std::vector<double>(y_axis_len, 0.0));
  std::vector<std::vector<double>> mean_x_array(
      x_axis_len, std::vector<double>(y_axis_len, 0.0));
  std::vector<std::vector<double>> mean_y_array(
      x_axis_len, std::vector<double>(y_axis_len, 0.0));
  ...
  for (size_t i = 0; i < max_gradient_vec.size(); i++) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      if (p1[0] >= 0 && p1[0] < x_axis_len && p1[1] >= 0 && p1[1] < y_axis_len &&
          p2[0] >= 0 && p2[0] < x_axis_len && p2[1] >= 0 && p2[1] < y_axis_len) {
...
void STDescManager::candidate_selector(
  ...
  for (size_t i = 0; i < stds_vec.size(); i++) {
    ...
    STDesc_LOC position;
    double dis_threshold = src_std.side_length_.norm() *
...
void STDescManager::candidate_verify(
  ...
  if (max_vote >= 4) {
    ...
    verify_score = static_cast<double>(max_vote) /
                   static_cast<double>(candidate_matcher.match_list_.size());
  } else {
    verify_score = -1;
  }
}
