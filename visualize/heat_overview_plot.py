...
def draw_heatmap(ax, data, title, vmin, vmax, cbar_label, fmt,
    ...
    cbar_ticks=None, cbar_tick_fmt=None, add_cbar=True, text_color='black'):
  ...
  # Show all axis labels without rotation for easier reading.
  ax.set_xticklabels([f'{v:.1f}' for v in beta_vals], fontsize=9)
  ax.set_yticklabels([f'{v:.1f}' for v in alpha_vals], fontsize=9)
  
  # Ensure all ticks are visible with minor ticks
  ax.tick_params(axis='x', which='major', labelsize=9, length=5, width=1)
  ax.tick_params(axis='y', which='major', labelsize=9, length=5, width=1)
  ...
  for i in range(data.shape[0]):
    for j in range(data.shape[1]):
      if np.isfinite(data[i, j]):
        cell_txt = format(data[i, j], fmt)
      else:
        cell_txt = ""
      ax.text(j, i, cell_txt, ha="center", va="center", fontsize=12, color=text_color)
