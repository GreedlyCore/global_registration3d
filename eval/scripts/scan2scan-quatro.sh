...
# Quatro sweep params.
NU=0.7
GRID_QUATRO_NOISE_BOUND_COEFFS=(0.2 0.5 1.0)
NOISE_BOUND_NU_SCALES=(0.2 0.4 0.6)
GRID_QUATRO_NOISE_BOUNDS=()
for s in "${NOISE_BOUND_NU_SCALES[@]}"; do
  GRID_QUATRO_NOISE_BOUNDS+=("$(awk -v nu="$NU" -v k="$s" 'BEGIN{printf "%.6f", nu*k}')")
done

# Optional pairwise mode (noise_bound_coeff, noise_bound)
PAIRWISE_QUATRO_NOISE_BOUND_COEFFS=(0.2 0.5 1.0)
PAIRWISE_QUATRO_NOISE_BOUNDS=(0.14 0.28 0.42)

# Backward-compatible aliases consumed by sweep_reg_methods.lib.sh.
GRID_ALPHA_RATIOS=("${GRID_QUATRO_NOISE_BOUND_COEFFS[@]}")
GRID_BETA_RATIOS=("${GRID_QUATRO_NOISE_BOUNDS[@]}")
PAIRWISE_ALPHA_RATIOS=("${PAIRWISE_QUATRO_NOISE_BOUND_COEFFS[@]}")
PAIRWISE_BETA_RATIOS=("${PAIRWISE_QUATRO_NOISE_BOUNDS[@]}")
...
if [[ "$PARAM_SWEEP_MODE" == "pairwise" ]]; then
  if [[ ${#PAIRWISE_QUATRO_NOISE_BOUND_COEFFS[@]} -ne ${#PAIRWISE_QUATRO_NOISE_BOUNDS[@]} ]]; then
    echo "[error] pairwise mode requires equal lengths: PAIRWISE_QUATRO_NOISE_BOUND_COEFFS, PAIRWISE_QUATRO_NOISE_BOUNDS" >&2
    exit 1
  fi
  if [[ ${#PAIRWISE_QUATRO_NOISE_BOUND_COEFFS[@]} -eq 0 ]]; then
...
if [[ "$PARAM_SWEEP_MODE" == "grid" ]]; then
  if [[ ${#GRID_QUATRO_NOISE_BOUND_COEFFS[@]} -eq 0 || ${#GRID_QUATRO_NOISE_BOUNDS[@]} -eq 0 ]]; then
    echo "[error] grid mode requires non-empty GRID_QUATRO_NOISE_BOUND_COEFFS and GRID_QUATRO_NOISE_BOUNDS" >&2
    exit 1
  fi
fi
